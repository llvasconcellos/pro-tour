...
public class frmReservaAerea extends JFrame implements Observer {

	private JPanel jpnControle, jpnAeroporto, jpnBotao;// painel que servira de container para os outros componentes da tela
	private JLabel lblNumero, lblEmpresa, lblEmail, lblDataIni, lblDataFim,
			lblHoraIni, lblHoraFim, lblOrigem, lblDestino, lblStatus,
			lblIntegrantes, lblAeroOrigem, lblAeroDestino, lblAssento;
	private JTextField txtNumero, txtEmpresa, txtEmail, txtOrigem, txtDestino,
			txtIntegrantes, txtAeroOrigem, txtAeroDestino, txtAssento;
	...
	public String getAeroOrigem(){
		return txtAeroOrigem.getText();
	}
	
	public String getAeroDestino(){
		return txtAeroDestino.getText();
	}
	...
	public frmReservaAerea() {

		setSize(600, 410);// tamanho tela
		setTitle("Reserva Aerea");// titulo tela
	...
	private void criaControle() {
		...
		jpnAeroporto = new JPanel(new ParagraphLayout());
		jpnAeroporto.setBorder(BorderFactory.createTitledBorder("Aeroporto"));
		add("Center", jpnAeroporto);

		lblAeroOrigem = new JLabel("Origem:");
		lblAeroOrigem.setForeground(Color.blue);
		jpnAeroporto.add(lblAeroOrigem, ParagraphLayout.NEW_PARAGRAPH);
		txtAeroOrigem = new JTextField(20);
		txtAeroOrigem.setEnabled(false);
		jpnAeroporto.add(txtAeroOrigem);
		
		lblAeroDestino = new JLabel("Destino:");
		lblAeroDestino.setForeground(Color.blue);
		jpnAeroporto.add(lblAeroDestino, ParagraphLayout.NEW_PARAGRAPH);
		txtAeroDestino = new JTextField(20);
		txtAeroDestino.setEnabled(false);
		jpnAeroporto.add(txtAeroDestino);
		
		lblAssento = new JLabel("Assento:");
		lblAssento.setForeground(Color.blue);
		jpnAeroporto.add(lblAssento, ParagraphLayout.NEW_PARAGRAPH);
		txtAssento = new JTextField(5);
		txtAssento.setEnabled(false);
		jpnAeroporto.add(txtAssento);
	...
	public void LimparTela() {
		...
		txtHoraIni.setText("");
		txtHoraFim.setText("");
		txtAeroOrigem.setText("");
		txtAeroDestino.setText("");
	...
	public void HabilitaCampos() {
		...
		txtAeroOrigem.setEnabled(true);
		txtAeroDestino.setEnabled(true);
							...
							if (campo.equalsIgnoreCase("destino")){
								...
								if (campo.equalsIgnoreCase("integrantes")){
									...
									if (campo.equalsIgnoreCase("aeroOrigem")){
										txtAeroOrigem.requestFocus();
									}else{
										if (campo.equalsIgnoreCase("aeroDestino")){
											txtAeroDestino.requestFocus();
										}else{
